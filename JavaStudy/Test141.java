...
○ String 클래스는 내부 문자열 데이터에 대한 *수정이 불가능*하다.
   (즉, 내용 불변 : Immutable)
   문자열의 내용이 변경되면 *새로운 객체를 생성*하며
   참조 대상을 잃어버린 객체는 가비지 컬렉션의 대상이 되고,
   새로운 객체에 새 주소를 할당하게 되므로
   해시코드도 변하게 된다.

○ String 클래스 간에 [+]연산자를 사용하게 회면
   문자열 간의 결합이 가능하며,
   ...
   ※ check~!!!
      눈으로 확인되는 결과는 이렇지만...
	  String은 내부 문자열의 내용을 변경할 수 없기 때문에
	  ...
	  StringBuffer나 cahr[] 을 사용하는 것이 효율적일 수 있다.
...
public class Test141
{
	public static void main(String[] args)
	{
		String s1 = "seoul";
		String s2 = "seoul";
		String s3 = new String("seoul"); // -> 위와 다르게 메모리에 있든 말든 무조건 메모리에 퍼올림
