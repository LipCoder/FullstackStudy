...
○ 프로그램에서 발생하는 오류(Error)는
   ...
   1. 잘못된 문법을 사용하거나 변수 등을 정의하지 않은 상태에서
      사용함으로써 *컴파일 단계*에서 발생하는 문법적인 오류(Error)
	  -> 컴파일 오류

   2. 프로그램을 *실행하는 과정*에서 발생되는 런타임 오류
      
	  - 개발자가 문제 분석을 잘못하거나 실수에 의해 
	    엉뚱한 결과를 가져오게 되는 논리적인 오류(Error)와

      - 시스템 이상에서 발생되는 시스템 오류(Error)(블루스크린), 그리고

	  - 프로그램 실행 중 발생되는 *비정상적인 상황*을 의미하는
	    예외사항(Exception)이 있다.
...
※	정리해 놓은 문법 (-> Exception 클래스)
	...
	- 프로그램 실행 중에 메소드 안에서 오류(Error)가 발생하게 될 경우,
	  ...
	  // 자바 런타임 시스템(Runtime System)에서 최종적으로 보고를 받는다.

	- 자바에서 모든 예외 클래스는 Throwable 클래스나 
	  Throwable 클래스의 하위 클래스를 상속받아 사용한다.

	- Throwable 클래스는 예외를 설명하는 문자이나
	  예외가 발생할 떄의 프로그램의 상태에 관한 정보를 포함하고 있다.
...
○ java.lang.Throwable 클래스의 주요 메소드
	...
	- void printStackTrace(PrintWriter w)
	    : 표준 출력 스트림에 스택 호울 목록을 마지막 메소드부터 출력한다.
...
○ 주요 런타임 예외 클래스
	...
	- ArrayStoreException
	   : 배열에 잘못된 데이터 형을 저장하려 앴을 경우 발생하는 오류
